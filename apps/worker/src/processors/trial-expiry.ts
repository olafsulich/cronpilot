...
export async function processTrialExpiry(
  ...
  const jobLog = logger.child({ jobId: job.id, teamId, processor: 'trial-expiry' })
...
async function scheduleWarnings(
  ...
  for (const { daysLeft, delayMs } of warnings) {
    ...
    await trialExpiryQueue.add(
      QUEUES.TRIAL_EXPIRY,
      { teamId, trialEndsAt: trialEndsAt.toISOString(), daysLeft } as TrialExpiryJobData & {
        trialEndsAt: string
        daysLeft: 3 | 1
      },
      {
