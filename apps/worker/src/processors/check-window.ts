...
import type { AlertJobData, CheckWindowJobData } from "@cronpilot/shared";
import { getNextWindowClose, QUEUES } from "@cronpilot/shared";
...
export async function processCheckWindow(
	...
	if (existingAlert) {
		...
		alertId = updated.id;
		jobLog.info(
			{ alertId, failureCount: updated.failureCount },
			"incremented failure count on existing alert",
		);
	} else {
