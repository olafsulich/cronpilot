...
export async function testIntegration(
  ...
  try {
    if (integration.type === 'slack') {
      ...
      if (!res.ok) {
        return { success: false, message: `Slack returned status ${res.status}` }
      }
      return { success: true }
    }
    ...
    if (integration.type === 'pagerduty') {
      ...
      if (!res.ok) {
        return { success: false, message: `PagerDuty returned status ${res.status}` }
      }
      return { success: true }
    }
    ...
    if (integration.type === 'email') {
      ...
      return { success: true, message: 'Email integrations are verified during delivery' }
    }
