...
async function authPlugin(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/auth/signup', async (request, reply) => {
    ...
    if (!body.success) {
      throw new AppError('VALIDATION_ERROR', body.error.errors.map((e) => e.message).join(', '), 400)
    }
    ...
    return reply.status(201).send({
      data: {
        ...
        team: { id: team.id, name: team.name, slug: team.slug, plan: team.plan },
      },
    })
  })
  ...
  fastify.post('/auth/refresh', async (request, reply) => {
    ...
    try {
      payload = fastify.jwt.verify<{ userId: string; teamId: string; type?: string }>(
        refreshToken,
        { secret: process.env['JWT_REFRESH_SECRET'] ?? 'refresh-secret' },
      )
    } catch {
