...
import type { AlertResolveJobData, SlackConfig, PagerDutyConfig, WebhookConfig, EmailConfig } from '@cronpilot/shared'
import { renderEmail, AlertResolvedEmail } from '@cronpilot/emails'
...
export async function processAlertResolve(job: Job<AlertResolveJobData>): Promise<void> {
  ...
  const jobLog = logger.child({ jobId: job.id, monitorId, alertId, processor: 'alert-resolve' })
  ...
  for (const rule of monitor.alertRules) {
    ...
    try {
      switch (integration.type) {
        case 'slack':
          await resolveSlack(config as SlackConfig, monitor.name, alert.failureCount, dashboardUrl, jobLog)
          break
        case 'pagerduty':
          await resolvePagerDuty(config as PagerDutyConfig, monitor.name, monitorId, jobLog)
          break
        case 'webhook':
          await resolveWebhook(config as WebhookConfig, monitor.id, monitor.name, alertId, alert.failureCount, jobLog)
          break
        case 'email':
          await resolveEmail(config as EmailConfig, monitor.name, alert.failureCount, dashboardUrl, jobLog)
          break
        default: {
